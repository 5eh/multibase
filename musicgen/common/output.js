...
export async function setupPaths(moduleName, outputArg = "./output") {
  ...
  const isRunningFromRoot = Deno.cwd().endsWith("musicgen") && !Deno.cwd().endsWith(moduleName);
    ...
    copyToRootOutput: async (sourceFilename, targetFilename = null) => {
      ...
      try {
        if (await exists(sourceFile)) {
          ...
          console.log(colors.yellow(`Warning: Source file not found: ${sourceFile}`));
          return false;
        }
      } catch (err) {
        console.warn(colors.yellow(`Warning: Could not copy to root output: ${err.message}`));
        return false;
      }
    }
  };
...
export async function compileTypst(templatePath, outputPath) {
  try {
    console.log(colors.blue(`Compiling Typst file: ${templatePath} to ${outputPath}`));
    ...
    if (typstOutput.code === 0) {
      console.log(colors.green(`Successfully compiled Typst file to: ${outputPath}`));
      return true;
    } else {
      const errorMessage = new TextDecoder().decode(typstOutput.stderr);
      console.log(colors.yellow(`Warning: Typst compilation had issues: ${errorMessage}`));
      return false;
    }
  } catch (typstError) {
    console.log(colors.yellow(`Warning: Could not run typst command: ${typstError.message}`));
    console.log(colors.yellow(`To generate the PDF, manually run: typst compile ${templatePath} ${outputPath}`));
    return false;
  }
}
