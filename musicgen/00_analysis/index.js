...
async function runAnalysis() {
  ...
  try {
    ...
    try {
      console.log(colors.blue("Attempting to connect to real GraphQL endpoint at http://localhost:4350/graphql..."));
      ...
      console.log(colors.yellow(`⚠️ Could not connect to real GraphQL endpoint: ${connectionError.message}`));
      console.log(colors.yellow("Falling back to mock data for demonstration purposes..."));
      ...
      for (let i = 0; i < 5000; i++) {
        const timestamp = new Date(jan2021.getTime() + Math.random() * (jan2021End.getTime() - jan2021.getTime()));
        mockData.data.transfers.push({
          timestamp: timestamp.toISOString(),
          blockNumber: Math.floor(Math.random() * 10000000) + 1000000
        });
      ...
      for (let i = 0; i < 8000; i++) {
        const timestamp = new Date(nov2024.getTime() + Math.random() * (nov2024End.getTime() - nov2024.getTime()));
        mockData.data.transfers.push({
          timestamp: timestamp.toISOString(),
          blockNumber: Math.floor(Math.random() * 10000000) + 1000000
        });
...
${usedRealData ? '*Using real blockchain data from GraphQL endpoint*' : '*Using synthetic demo data (no GraphQL endpoint available)*'}
`;
    ...
    if (paths.isRunningFromRoot) {
      await Deno.writeTextFile(paths.getRootOutputPath('analysis.json'), JSON.stringify(jsonData, null, 2));
    }
    ...
    await Deno.writeTextFile(paths.getTypstPath('analysis.json'), JSON.stringify(jsonData, null, 2));
    ...
    const compileSuccess = await compileTypst(reportTemplatePath, reportPdfPath);
