...
export async function GET(request) {
  try {
    ...
    try {
      ...
      console.error(`Directory ${lyricsDir} does not exist or is not accessible:`, error);
    ...
    if (slug) {
      ...
      const partialMatch = !exactMatch ? markdownFiles.find(file => 
      ...
      if (matchedFile) {
        ...
        try {
          ...
          const content = await fs.readFile(path.join(lyricsDir, matchedFile), 'utf-8');
