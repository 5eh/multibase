...
function createTimeline() {
  ...
  const verticalSpaceBetweenMarkers = Math.max(
    30,
    (timelineHeight - 120) / totalMonths,
...
function preloadNearbyMonths(currentIndex) {
  ...
  for (let offset = -maxPreload; offset <= maxPreload; offset++) {
    ...
    if (nearbyIndex >= 0 && nearbyIndex < markers.length) {
      ...
      const nearbyMonth = nearbyDate.toLocaleDateString("en-US", {
        month: "long",
      }).toLowerCase();
...
function playAudio() {
  ...
  audioLoader.load(
    ...
    function (error) {
      ...
      console.log(
        "Could not load month-specific music, falling back to default",
        error,
      );
...
function updateDownloadLinks(dateString) {
  ...
  if (imageCache.has(coverUrl)) {
    ...
    thumbnailImg.src =
      'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white"><path d="M12 4V2C6.48 2 2 6.48 2 12h2c0-4.41 3.59-8 8-8zm0 14c-3.31 0-6-2.69-6-6H4c0 4.41 3.59 8 8 8v-2zm0-12V4c-3.31 0-6 2.69-6 6h2c0-2.21 1.79-4 4-4zm0 12c2.21 0 4-1.79 4-4h-2c0 1.1-.9 2-2 2v2z"/></svg>';
    ...
    preloadImage(coverUrl)
      ...
      .catch((error) => {
        ...
        thumbnailImg.src =
          'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>';
      });
  }
}
...
function initWebGL() {
  try {
    ...
    fallbackMsg.innerHTML =
      "<h2>WebGL not supported by your browser</h2><p>Please try a different browser or device</p>";
...
function initScene() {
  ...
  function animate() {
    ...
    if (
      analyser && sound && sound.isPlaying && uniforms && uniforms.u_frequency
    ) {
...
try {
  if (renderer && params && typeof GUI === "function") {
    ...
    if (uniforms) {
      ...
      colorsFolder.add(params, "red", 0, 1).name("Red").onChange(
        function (value) {
      ...
      colorsFolder.add(params, "green", 0, 1).name("Green").onChange(
        function (value) {
      ...
      colorsFolder.add(params, "blue", 0, 1).name("Blue").onChange(
        function (value) {
    ...
    if (bloomPass) {
      ...
      bloomFolder.add(params, "threshold", 0, 1).name("Threshold").onChange(
        function (value) {
          if (bloomPass) {
      ...
      bloomFolder.add(params, "strength", 0, 5).name("Strength").onChange(
        function (value) {
          if (bloomPass) {
      ...
      bloomFolder.add(params, "radius", 0, 2).name("Radius").onChange(
        function (value) {
          if (bloomPass) {
